...
* If the shot was assisted, then there shall be another `+` followed by the jersey number of the player who made the assist (as this is always a team member of the shooter, the team (A or B) is not separately specified). An assist shall be recorded on exactly one scoring event per play: if the field goal is made (including and-1 situations), the assist is on the field goal; if the field goal is missed but a shooting foul results in free throws, the assist is on the first successful free throw only
